...
<?php
  ...
  ORDER BY name ASC;";
    ...
    //calculate how many movies the system should recommend
      ...
      $movie_query = "SELECT m.title, m.date_released, w.movie_id, AVG(rating) FROM movie_rater.movie m,
     movie_rater.tag t, movie_rater.movie_tags mt, movie_rater.watches w WHERE t.tag_id = '$tag_row[1]'
      AND t.tag_id = mt.tag_id AND mt.movie_id =  m.movie_id AND m.movie_id = w.movie_id AND NOT w.user_id ='$user'
      GROUP BY m.title, m.date_released, w.movie_id
